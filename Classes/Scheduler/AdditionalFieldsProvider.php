...
class AdditionalFieldsProvider implements \TYPO3\CMS\Scheduler\AdditionalFieldProviderInterface {
	...
	public function getAdditionalFields(array &$taskInfo, $task, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $schedulerModule) {
		...
		$additionalFields['domainRecord'] = array(
			'code'     => '<select class="wide form-control" type="text" name="tx_scheduler[domainRecord]">'. $this->buildSelectItems($task->getDomainRecordId()) .'</select>',
			'label'    => 'LLL:EXT:dd_googlesitemap/locallang.xml:scheduler.domainRecordLabel',
			'cshKey'   => '',
			'cshLabel' => ''
		);
		$additionalFields['eIdUrl'] = array(
			'code'     => '<textarea style="width:350px;height:200px" class="form-control" name="tx_scheduler[eIdUrl]" wrap="off">' . htmlspecialchars($url) . '</textarea>',
